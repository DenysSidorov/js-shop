...
function* signInUserSaga(action: any) {
  ...
  try {
    const response: ResponseGenerator = yield call(signInAPI, action.payload.login, action.payload.password);
...
function* isAdminSaga() {
  try {
    ...
    const token: ResponseGenerator = yield call([localStorage, localStorage.getItem], 'info');
    // eslint-disable-next-line
    const response: ResponseGenerator = yield call(isAdminAPI, token);
